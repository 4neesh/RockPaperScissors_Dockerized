...
class GameMode(Enum):
    ...
    def __init__(self, humans: int, computers: int):
        ...
        if self.humans + self.computers != 2:
            raise ValueError(f"Invalid Game Mode created, all modes must be set-up for exactly 2 players")
    ...
    def formatted_choices() -> list[str]:
        return [f"\n{index} = {mode.name}" for index, mode in enumerate(GameMode, start =1)]
